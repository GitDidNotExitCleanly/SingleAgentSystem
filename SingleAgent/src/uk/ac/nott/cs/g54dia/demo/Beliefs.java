...
public class Beliefs {
	...
	// find the nearest well for each station and store station-well pair in hashmap
	...
	private void generateOptimalTaskList() {
		...
		// density of tasks with the same nearest well
	...
	// exploration point optimization
		...
		if (WhetherReplan) {
			...
			for (int i=0;i<width;i++) {
				for (int j=0;j<height;j++) {
					if (min_X+i*Tanker.VIEW_RANGE*2+min_X+(i+1)*Tanker.VIEW_RANGE*2 != 0 && max_Y-j*Tanker.VIEW_RANGE*2+max_Y-(j+1)*Tanker.VIEW_RANGE*2 != 0) {
			...
			this.optimalExplorationPoints.add(new myPoint(0,0));
			for (int i=0;i<4;i++) {
				this.optimalExplorationPoints.addAll(tensity.get(i));
			}
	...
	public ArrayList<myPoint> getOptimalExplorationPoints() {
		return this.optimalExplorationPoints;
	}
	...
	public void updateBeliefs(Cell[][] view,int fuel,int water, Cell currentCell) {
		...
		// is current position one of exploration position ?
		if (this.explorationPoints.remove(currentPosition)) {
